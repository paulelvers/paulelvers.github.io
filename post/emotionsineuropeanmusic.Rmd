...
eutop50 <- read.csv2("~/Documents/Data Science Projects/Spotify/euro2018-01-24.csv")

```

```{r, echo=FALSE,  error=F, message=F, warning=FALSE, fig.height = 14, fig.width = 12}
# loading libraries
### --------------------------------------------------
library(highcharter)
library(scales)
...
map_stats<-eutop50 %>%
  ...
  summarise(val = mean(valence, na.rm = T))
...
balearic[!balearic_index, ] %>%
  ...
  left_join(unique(eutop50[,4:6], by="track_uri")) %>%
...
```{r, echo = F, error = F, message = F, warning = FALSE, fig.height = 5, fig.width = 6}
# loading libraries
### --------------------------------------------------
library(forcats)
library(ggplot2)
...
``` {r, echo = F, error = F, message = F, warning = F, fig.height = 5, fig.width = 6}
